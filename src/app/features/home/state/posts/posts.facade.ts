...
export class PostsFacade {
  ...
  public async loadPosts(_payload: PostRequestModel): Promise<void> {
    await this.store.dispatch(new LoadPosts(_payload));
  }

  public async loadPopularPosts(_payload: PostRequestModel): Promise<void> {
    await this.store.dispatch(new LoadPopularPosts(_payload));
  }
}
